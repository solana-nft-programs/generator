...
test-keys:
	anchor build
	LC_ALL=C find programs src -type f -exec sed -i '' -e "s/mdg7hsS3aWuWwSGVFTgC6KWpCaDJZ5qbEgWBQoGX4id/$$(solana-keygen pubkey ./target/deploy/cardinal_metadata_generator-keypair.json)/g" {} +
	anchor build
...
start:
	solana-test-validator --url https://api.devnet.solana.com \
		...
		--bpf-program ./target/deploy/cardinal_metadata_generator-keypair.json ./target/deploy/cardinal_metadata_generator.so \
...
clean-test-keys:
	LC_ALL=C find programs src -type f -exec sed -i '' -e "s/$$(solana-keygen pubkey ./target/deploy/cardinal_metadata_generator-keypair.json)/mdg7hsS3aWuWwSGVFTgC6KWpCaDJZ5qbEgWBQoGX4id/g" {} +
